...
// ✅ IMPORTANT: This prevents Next/Vercel from trying to prerender this page at build time.
// It fixes the Vercel build error caused by useSearchParams().
...
export default function EducationBookingPage() {
  ...
  // Keep message prefilled from the selected program details
  useEffect(() => {
  ...
  async function onSubmit(e) {
    ...
    try {
      // Include prefill details in payload too (useful for the email body)
      const payload = {
        ...form,
        prefill, // { country, level, intake, university }
      };
    ...
    <main className="bg-white">
      <section className="mx-auto max-w-3xl px-4 py-14">
        ...
        <div className="mt-6 rounded-3xl border border-border bg-zinc-50 p-5 text-sm text-muted-foreground">
          <div>
            <b>Country:</b> {prefill.country || "-"}
          </div>
          <div>
            <b>University:</b> {prefill.university || "-"}
          </div>
          <div>
            <b>Level:</b> {prefill.level || "-"}
          </div>
          <div>
            <b>Intake:</b> {prefill.intake || "-"}
          </div>
        </div>
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-4">
          ...
          {/* ✅ Conditionally show extra input */}
          {needsReferral && (
            <Field
              label={
                form.heardAbout === "Other"
                  ? "Please specify"
                  : "Referral name (who referred you?)"
              }
            >
              <input
                className="input"
                ...
                placeholder={
                  form.heardAbout === "Other" ? "Where did you find us?" : "e.g., Chinedu Okafor"
                }
              />
            </Field>
          )}
...
//                   ...prev,
//                   heardAbout: next,
//                   // clear referral name if they switch away
//                   referralName:
//                     next === "Friend / Referral (Person)" || next === "Other"
//                       ? prev.referralName
//                       : "",
...
//     service: "Education & Study Abroad",
//     destination: prefill.country || "Other",
//     message: `University: ${prefill.university}\nLevel: ${prefill.level}\nIntake: ${prefill.intake}\n\nAdditional details:`,
//   });
...
//           <Field label="Message">
//             <textarea className="input min-h-[160px]" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} />
//           </Field>
...
//         <div className="mt-8">
//           <Link href="/education" className="text-sm font-semibold text-primary">← Back to Education</Link>
//         </div>
